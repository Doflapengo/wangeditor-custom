...
class Head extends BtnMenu implements MenuActive {
    ...
    constructor(editor: Editor) {
        const $elem = $('<div class="w-e-menu" data-title="正文">正文</div>')
        super($elem, editor)
        // 绑定事件
        bindEvent(editor)
    ...
    public command(value: string): void {
        ...
        window.jq(tagName).find('span').remove()
        $(editor.selection?.getSelectionContainerElem()?.elems[0]).removeClass($(tagName).attr('class'))
    }
